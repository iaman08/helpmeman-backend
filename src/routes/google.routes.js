...
router.get('/oauth/url', authenticate, roleGuard('MENTOR'), async (req, res) => {
  try {
...
router.get('/oauth/status', authenticate, roleGuard('MENTOR'), async (req, res) => {
  try {
...
router.delete('/oauth/disconnect', authenticate, roleGuard('MENTOR'), async (req, res) => {
  try {
    const mentor = await prisma.mentor.findUnique({ where: { userId: req.user.id } });
    if (!mentor) return res.status(404).json({ error: 'Mentor profile not found' });
...
router.put('/calendar/timezone', authenticate, roleGuard('MENTOR'), async (req, res) => {
  try {
